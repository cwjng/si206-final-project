...
if num_rows < 100:
    ...
    for item in results['artists']['items']:
        ...
        c.execute("INSERT OR IGNORE INTO top_artists (id, name, popularity, followers) VALUES (?, ?, ?, ?)",
        ...
        c.execute("INSERT INTO song_info (artist_id, name, top_song, popularity) VALUES (?, ?, ?, ?)",
                  (artist_id, artist_name, top_track_name, top_track_popularity))

    # Commit the changes to the database and close the connection
    conn.commit()
    conn.close()

    # Wait for a few seconds before running the code again
    time.sleep(5)
